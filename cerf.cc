...
namespace faddeeva_impl {
	EXEC_TARGET static inline void cexp(fptype& re, fptype& im)
	{
		const fptype e = EXP(re);
		re = e * COS(im);
		im = e * SIN(im);
	}

    template <class T, unsigned N, unsigned NTAYLOR, unsigned NCF>
    EXEC_TARGET_DEV static inline devcomplex<T> faddeeva_smabmq_impl(
	...
	if (zim2 < maxnorm) {
		...
		const int nsing = int(FABS(dnsing) + T(1) / T(2));
		// and calculate just how far we are from it
		const T zmnpire = FABS(zre) - npi[nsing];
		...
		if (zmnpinorm < maxnorm) {
		    ...
		    if (negrez) return devcomplex<T>(sumre, -sumim);
		    else return devcomplex<T>(sumre, sumim);
		}
	    }
	}
	...
	if (znorm > tm * tm) {
		...
		return devcomplex<T>(T(2) * ez2re - sumre,
			T(2) * ez2im - sumim);
	    } else {
		return devcomplex<T>(sumre, sumim);
	    }
	}
...
//#endif
	...
	if (negimz) {
	    ...
	    T ez2re = z2re, ez2im = z2im;
	    faddeeva_impl::cexp(ez2re, ez2im);
	    return devcomplex<T>(T(2) * ez2re + sumim / twosqrtpi,
		    T(2) * ez2im - sumre / twosqrtpi);
	} else {
	    return devcomplex<T>(-sumim / twosqrtpi, sumre / twosqrtpi);
	}
    }

    static const fptype npi24[24] = { // precomputed values n * pi
    ...
    static const fptype a24[24] = { // precomputed Fourier coefficient prefactors
    ...
    static const fptype taylorarr24[24 * 12] = {
    ...
    const fptype npi11[11] = { // precomputed values n * pi
	0.00000000000000000e+00, 3.14159265358979324e+00, 6.28318530717958648e+00,
	9.42477796076937972e+00, 1.25663706143591730e+01, 1.57079632679489662e+01,
	1.88495559215387594e+01, 2.19911485751285527e+01, 2.51327412287183459e+01,
	2.82743338823081391e+01, 3.14159265358979324e+01
    };
    const fptype a11[11] = { // precomputed Fourier coefficient prefactors
...
EXEC_TARGET devcomplex Cerf::faddeeva(devcomplex z)
{
    return faddeeva_impl::faddeeva_smabmq_impl<fptype, 24, 6, 9>(
	    z.real(), z.imag(), 12., faddeeva_impl::a24,
	    faddeeva_impl::npi24, faddeeva_impl::taylorarr24);
}

EXEC_TARGET devcomplex Cerf::faddeeva_fast(devcomplex z)
{
    return faddeeva_impl::faddeeva_smabmq_impl<fptype, 11, 3, 3>(
	    z.real(), z.imag(), 8., faddeeva_impl::a11,
	    faddeeva_impl::npi11, faddeeva_impl::taylorarr11);
}

EXEC_TARGET devcomplex Cerf::erfc(const devcomplex z)
{
    fptype re = -z.real() * z.real() + z.imag() * z.imag();
    fptype im = -2. * z.real() * z.imag();
    faddeeva_impl::cexp(re, im);
    return (z.real() >= 0.) ?
	(devcomplex(re, im) *
	 faddeeva(devcomplex(-z.imag(), z.real()))) :
	(2. - devcomplex(re, im) *
	 faddeeva(devcomplex(z.imag(), -z.real())));
}

EXEC_TARGET devcomplex Cerf::erfc_fast(const devcomplex z)
{
    fptype re = -z.real() * z.real() + z.imag() * z.imag();
    fptype im = -2. * z.real() * z.imag();
    faddeeva_impl::cexp(re, im);
    return (z.real() >= 0.) ?
	(devcomplex(re, im) *
	 faddeeva_fast(devcomplex(-z.imag(), z.real()))) :
	(2. - devcomplex(re, im) *
	 faddeeva_fast(devcomplex(z.imag(), -z.real())));
}

EXEC_TARGET devcomplex Cerf::erf(const devcomplex z)
{
    fptype re = -z.real() * z.real() + z.imag() * z.imag();
    fptype im = -2. * z.real() * z.imag();
    faddeeva_impl::cexp(re, im);
	...
	(1. - devcomplex(re, im) *
	 faddeeva(devcomplex(-z.imag(), z.real()))) :
	(devcomplex(re, im) *
	 faddeeva(devcomplex(z.imag(), -z.real())) - 1.);
}

EXEC_TARGET devcomplex Cerf::erf_fast(const devcomplex z)
{
    fptype re = -z.real() * z.real() + z.imag() * z.imag();
    fptype im = -2. * z.real() * z.imag();
    faddeeva_impl::cexp(re, im);
	...
	(1. - devcomplex(re, im) *
	 faddeeva_fast(devcomplex(-z.imag(), z.real()))) :
	(devcomplex(re, im) *
	 faddeeva_fast(devcomplex(z.imag(), -z.real())) - 1.);
}
